...
def random_rotation(x, rg, y=None,row_index=1, col_index=2, channel_index=0,
...
def random_shift(x, wrg, hrg, y=None,row_index=1, col_index=2, channel_index=0,
                 fill_mode='nearest', cval=0.):
    h, w = x.shape[row_index], x.shape[col_index]
...
def random_zoom(x, zoom_range, row_index=1, col_index=2, channel_index=0,
                fill_mode='nearest', cval=0., transform_matrix=None):
    if len(zoom_range) != 2:
...
class ImageDataGenerator(object):
    ...
    def random_transform(self, x, y=None):
        ...
        if y is not None and self.transform_label:
        ...
        if self.elastic and self.elastic_label:
            if np.random.random() < 0.5:
                if y is not None and self.transform_label:
    ...
    def _flow_index(self, N, batch_size=32, shuffle=True, seed=None):
        ...
        while 1:
            ...
            if chunkidx == numberofchunk-1:
                chunkidx = 0
                number_repeat += 1
                batch_end = False # because we dont want stopiteration before processing this, use this as a marker.
                current_index = 0
            else:
