...
class ListIterator(Iterator):
    ...
    def __init__(self, fileList, labelList, image_data_generator, directory = '',
                 target_size=(256, 256), color_mode='rgb',
                 dim_ordering=K.image_dim_ordering,
                 classes=None, class_mode='categorical',
                 batch_size=32, shuffle=True, seed=None,
        ...
        self.nb_sample = len(classes)   
        if type(class) is list:
            self.nb_class = len(classes[0])
        else:
           self.nb_class = max(classes)
        ...
        self.classes = np.zeros((self.nb_sample,), dtype='int32')
        i = 0
        for index, thislist in enumerate(self.fileList):
            self.classes[i] = self.class_indices[subdir]
        ...
        for subdir in classes:
            ...
            for fname in os.listdir(subpath):
                ...
                if is_valid:
                    ...
                    self.filenames.append(os.path.join(subdir, fname))
                    i += 1
        super(ListIterator, self).__init__(self.nb_sample, batch_size, shuffle, seed)
